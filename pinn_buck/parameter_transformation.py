...
def make_log_param(params: Parameters, rescaling: Optional[Parameters] = None) -> Parameters:
    """
    ...
    def _to_log(
        value: Union[float, torch.Tensor], scale: Union[float, torch.Tensor] = 1.0
    ) -> torch.Tensor:
        v = torch.as_tensor(value, dtype=torch.float32)
        s = torch.as_tensor(scale, dtype=torch.float32)
        return torch.log(v * s)

    # safer: align by names, not just zip order  # e.g. {"L": sL, "RL": sRL, ...}
