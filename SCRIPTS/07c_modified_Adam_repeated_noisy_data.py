...
def calculate_L2_weights(X: torch.Tensor, y: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Calculate weights for the loss function based on the input data.
    """
    # if the distance between X[:, :2] and y is small the weight should be smaller
    i_distance = torch.norm(X[:, :1] - y[:, :1], dim=1)
    v_distance = torch.norm(X[:, 1:2] - y[:, 1:2], dim=1)

    # calculate weights inversely proportional to the distance
    weights_i = i_distance**2 / (torch.sum(i_distance**2) + torch.sum(v_distance**2) + 1e-8)
    weights_v = v_distance**2 / (torch.sum(v_distance**2) + torch.sum(i_distance**2) + 1e-8)

    return weights_i, weights_v

# def calculate_L1_weights(X: torch.Tensor, y: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
#     """
#     Calculate weights for the L1 loss function based on the input data.
...
#     # calculate weights inversely proportional to the distance
#     weights_i = i_distance / (torch.sum(i_distance) + torch.sum(v_distance) + 1e-8)
#     weights_v = v_distance / (torch.sum(v_distance) + torch.sum(i_distance) + 1e-8)
#     return weights_i, weights_v
...
#     # L1 regularization on the predictions
#     return (
#         (weights_i * torch.abs(i_n - i0)).sum()
#         + (weights_v * torch.abs(v_n - v0)).sum()
#         + (weights_i * torch.abs(i_np1 - i1)).sum()
#         + (weights_v * torch.abs(v_np1 - v1)).sum()
#     )
...
def train_from_measurement_file(
    ...
    for it in range(epochs1 + epochs2):
        ...
        if it < epochs1:
            # use L2 loss for the first phase of training
...
noise_levels = [
    ...
    5, 
    10
] # MORE CONTROL ON WHICH MEASUREMENTS TO USE 
...
for idx, noise in enumerate(noise_levels):
    print(f"\n{'-'*50}")
    print(f"{idx}) Training with noise level: {noise}")

    measurements = noisy_measurements[noise]

    number_of_noise_samples = len(measurements)
    for idx in range(number_of_noise_samples):
        ...
        train_from_measurement_file(
            measurements[idx],
            db_dir=out_dir,
            savename=f"noisy_run_{noise}_{idx+1}.csv",
            lr=lr,
            patience=patience,
