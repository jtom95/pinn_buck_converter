...
class PINNBuck(nn.Module):
    ...
    def forward(self, X: torch.Tensor):
        S, dt = X[:, 2:3], X[:, 3:4]
        out = self.net(self._scale(X))
        i_k, v_k = out[:, : self.q], out[:, self.q : 2 * self.q]
        ...
        di = self._di(i_k, v_k, S, p)
        dv = self._dv(i_k, v_k, S, p, rload, di)

        i_n = i_k - dt * (di @ self.irk_alpha.T)
        v_n = v_k - dt * (dv @ self.irk_alpha.T)
        i_np1 = i_k + dt * (di @ (self.irk_beta - self.irk_alpha).T)
        v_np1 = v_k + dt * (dv @ (self.irk_beta - self.irk_alpha).T)
...
for it in range(epochs):
    adam.zero_grad()
    loss = compute_loss(model(X_t), x0, y_t)
    loss.backward()
    adam.step()
    if it % 1000 == 0:
...
history_file = save_dir / "history_pinn_buck.csv"
