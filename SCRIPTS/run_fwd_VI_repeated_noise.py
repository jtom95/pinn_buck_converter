...
lsb_i = I_FS / (2**12 - 1)  # 10 A full-scale current
lsb_v = V_FS / (2**12 - 1)  # 30 V full-scale voltage
...
noise_power_dict = {
    "1LSB": (noise_power_ADC_i, noise_power_ADC_v),
    "5LSB": (noise_power_5_i, noise_power_5_v),
    "10LSB": (noise_power_10_i, noise_power_10_v),
}
...
set_seed(123)
device = "cpu"
out_dir = Path.cwd() / "RESULTS" / "REPEATED_NOISE" / "LIKELIHOODS" / "VIF"
...
trained_models = {}
trained_runs = {}
laplace_posteriors: Dict[str, Dict[int, LaplacePosterior]] = {}
...
for ii, label in enumerate(noise_power_dict.keys()):
    lsb = int(label.replace("LSB", "").strip())

    print(f"\n{'-'*50}")
    print(f"{ii}) Training with {lsb}LSB")
    
    laplace_posteriors[label] = {}
    trained_models[label] = {}
    trained_runs[label] = {}
    
    for jj in range(n):
        print(f"\tRun #{jj}")
        noisy_measurement = add_noise_to_Measurement(
            io.M, V_FS=V_FS, I_FS=I_FS, noise_level=lsb
        )

        # Train the model on the noisy measurement
        X = torch.tensor(noisy_measurement.data, device=device)
        model = BuckParamEstimator(param_init = ParameterConstants.NOMINAL).to(device)
        ...
        callback_func = partial(
            ...
            data_covariance=noise_power_dict[label]
        )
        ...
        out_dir_jj = out_dir / str(jj)
        out_dir_jj.mkdir(parents=True, exist_ok=True)
        
        laplace_posterior.save(out_dir_jj / f"laplace_posterior_{label}#{jj}")
 

        laplace_posteriors[label][jj] = laplace_posterior
        trained_models[label][jj] = trainer.optimized_model()
        trained_runs[label][jj] = trainer.history

        trainer.history.get_best_parameters().save(out_dir_jj / f"best_params_{label}#{jj}")
        trainer.history.save_to_csv(out_dir_jj / f"history_{label}#{jj}.csv")
