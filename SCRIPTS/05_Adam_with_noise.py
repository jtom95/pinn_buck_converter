...
## Test the training on different noise levels
ideal_meas = io.M
...
out_dir = Path(__file__).parent.parent / "RESULTS" / "removed_nn" / "noisy_runs"
...
for idx, noise in enumerate(noise_levels):
    ...
    noisy_meas = add_noise_to_Measurement(
        ideal_meas, 
        noise_level=noise, 
        scale_transients_together=True
    )
    ...
    train_from_measurement_file(
        noisy_meas,
        db_dir=out_dir,
        savename=f"noisy_run_{noise}.csv",
        lr=lr,
        patience=patience,
