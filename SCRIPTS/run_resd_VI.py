...
from pinn_buck.data_noise_modeling.covariance_matrix_function_archive import covariance_matrix_on_basic_residuals, covariance_matrix_on_residuals_d1, generate_residual_covariance_matrix, chol
...
from pinn_buck.model.residuals import basic_residual, residual_d1
...
def calculate_Lr_and_vif(model: BaseBuckEstimator, X: torch.Tensor, data_covariance: Tuple[float, float]) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    jacobian_estimator = FwdBckJacobianEstimator()
    jac_fwd = jacobian_estimator.estimate_Jacobian(
        X, model, direction="forward", number_of_samples=500, dtype=torch.float64
    )[..., :2, :2]
    jac_bck = jacobian_estimator.estimate_Jacobian(
        X, model, direction="backward", number_of_samples=500, dtype=torch.float64
    )[..., :2, :2]
    covariance_matrix = generate_residual_covariance_matrix(
        data_covariance=data_covariance,
        residual_covariance_func=covariance_matrix_on_residuals_d1,
    ...
    with torch.no_grad():
        ...
        residuals = residual_d1(pred, targets)
...
for label, data_covariance in noise_power_dict.items():
    ...
    with torch.no_grad():
        targets = model.targets(X)
        preds = model(X)
        residuals = residual_d1(preds, targets)
    # plot the residuals
...
set_seed(123)
device = "cpu"
out_dir = Path.cwd() / "RESULTS" / "LIKELIHOODS" / "RESD1_VIF"
