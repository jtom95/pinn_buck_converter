...
PRIOR_SIGMA = rel_tolerance_to_sigma(REL_TOL)
...
for ii, ((name, nominal), (_, sigma), (_, true_val)) in enumerate(zip(NOMINAL, PRIOR_SIGMA, TRUE_PARAMS)):
...
for idx, group_number in enumerate(l_dict.keys()):
    ...
    print(
        f"True Param Loss for {group_name} (diagonal noise): {loss_diag_ideal:.6e} vs best found: {trainer.history.get_best_loss('Adam'):.6e}"
    )
    print(
        f"True Param Loss for {group_name} (full noise): {loss_full_ideal:.6e} vs best found: {trainer.history.get_best_loss('LBFGS'):.6e}"
    )
    ...
    print(
        TEMPLATE.format(
            idx=3, label="Loss for best with DIAGONAL COVARIANCE:", value=loss_full_best_Adams
        )
    )
