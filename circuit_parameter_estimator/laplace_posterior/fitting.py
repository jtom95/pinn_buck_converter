...
from ..optimization_loss.map_loss import MAPLoss
...
class LaplacePosterior:    
    ...
    param_units: Dict[str, str]  # e.g. {"L":"H", "RL":"Ω", ...}
    ...
    def print_param_uncertainty(
        ...
        if distribution_type == "gaussian":
            ...
            for name, dist in zip(self.param_names, dists):
                ...
                print(f"{name:10s}: {mean:.3e}  ±{wid:.1e} {self.param_units.get(name, '')} ({pct:.2f} %)")
            ...
            for name, dist in zip(self.param_names, dists):
                ...
                print(
                    f"{name:10s}: {mean:.3e}  "
                    f"-{minus:.1e}/+{plus:.1e}  {self.param_units.get(name, '')} "
...
class LaplaceApproximator:
    ...
    @property
    def param_units(self) -> Dict[str, str]:
        """List of parameter units in the order matching the flat vectors."""
        units = self.model.get_estimates().units
        return {name: units.get_from_iterator_name(name, "") for name in self.param_names}  
