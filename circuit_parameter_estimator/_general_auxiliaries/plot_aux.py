from typing import Iterable, Tuple, Optional, Sequence, Callable
...
def make_line_with_center_marker_from_dict(marker_dict: dict[str, str]):
    def _transform(h: Artist, label: str) -> Artist:
        if isinstance(h, Line2D):
            mk = marker_dict.get(label, None)
            if mk:
                return Line2D(
                    [],
                    [],
                    ...
                    color=h.get_color(),
                    marker=mk,
                    markersize=h.get_markersize() or 6,
                    markerfacecolor=h.get_color(),
                    markeredgecolor=h.get_color(),
                )
        return h
...
def line_with_center_marker(h: Artist, _label: str, marker="o") -> Artist:
    if isinstance(h, Line2D):
        return Line2D(
            [],
            [],
            ...
            linewidth=h.get_linewidth(),
            color=h.get_color(),
            marker=marker,  # centered marker on legend line
            markersize=6,
            markerfacecolor=h.get_color(),
            markeredgecolor=h.get_color(),
        )
...
def place_shared_legend(
    ...
    *,
    ...
    legend_handles: Optional[Sequence[Artist]] = None,
    legend_labels: Optional[Sequence[str]] = None,
    handle_transform: Optional[Callable[[Artist, str], Artist]] = None,
) -> None:
    """
    ...
    if legend_handles is not None and legend_labels is not None:
        ...
        # collect unique handles/labels  (YOUR EXISTING BLOCK)
    ...
    if handle_transform is not None:
        handles = [handle_transform(h, l) for h, l in zip(handles, labels)]
